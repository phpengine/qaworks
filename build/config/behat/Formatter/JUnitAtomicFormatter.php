 ...
 */
class JUnitAtomicFormatter extends ConsoleFormatter
{
    /**
...
//        $this->scenarioStepsCount = 0;
        ...
        $this->filename = 'NUK-TEST-'.getenv('NU_EXEC_PLATFORM').$this->generateRandomString() . strtr($scenario->getFile(), $replace);

        $this->printTestSuiteHeader($scenario);
    ...
    public function afterScenario(ScenarioEvent $event)
    {
        ...
        ++$this->scenariosCount;
        $this->printTestSuiteFooter($event->getScenario(), microtime(true) - $this->scenarioStartTime);
        $this->flushOutputConsole();
     ...
     *
     * @param FeatureNode $scenario
     */
    protected function printTestSuiteHeader(ScenarioNode $scenario)
    {
     ...
     * @param float       $time
     */
    protected function printTestSuiteFooter(ScenarioNode $scenario, $time)
    {
        $suiteStats = sprintf('errors="0" failures="%d" skipped="%d" name="%s" tests="%d" time="%F"',
            $this->exceptionsCount,
            $this->pendingCount,
            htmlspecialchars($scenario->getTitle()),
            $this->scenariosCount,
            $time
        );
